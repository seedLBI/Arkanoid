...
void Game::DrawDebug(
	...
	border.Draw(quads_renderer,triangles_renderer);
...
void Game::Draw(TriangleInstanced& triangles_renderer, QuadInstanced& quads_renderer, DebugCircle& circles_renderer, TextInstanced& text_renderer) {
	...
	score.Draw(quads_renderer, text_renderer, *font, {0,0}, TranslateScalar_GlobalToScreen(0.2f));
	combo.Draw(text_renderer, *font, TranslateGlobalToScreen({ -1.f,0.6f }), TranslateScalar_GlobalToScreen(0.2f));
}
void Game::Update() {
	UpdateAnimValues();
	player.Update();
...
std::optional<CollisionInfo> Game::TryCollision(Ball& ball_, const AABB_Region& aabb, const std::vector<glm::vec2>& vertices, int collision_push_type) {
	...
	return TryCollision(ball_, vertices, collision_push_type);
...
bool Game::ResolveCollision(Ball& ball_) {
	...
	while (count_recals <= max_recalc) {
		...
		if (collisionData.info.has_value()) {
			ApplyCollision(ball_, collisionData);
			ReactToCollision(collisionData);
		}
		else {
			break;
		}
...
void Game::ApplyCollision(Ball& ball_, const ClosestCollisionData& data) {
	const CollisionInfo& collision = data.info.value();
...
void Game::ReactToCollision(const ClosestCollisionData& data) {
	...
	case COLLISION_OBJECT_TYPE::COLLISION_DESTROYABLE:
		...
		combo.AddCombo();
		score.AddScore(20.0 * combo.GetValue());
